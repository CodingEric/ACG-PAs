...
mu_0, lambda_0 = E / (2 * (1 + nu)), E * nu / (
    (1 + nu) * (1 - 2 * nu))  # Lame parameters from mpm99
...
def substep(gravity: float):
    ...
    for p in x:
        if is_used[p]:  # LOOKATME: do not swap the branch stmt with the for-loop, because ONLY the outermost loop stmt can be parallized.
            ...  # do something here
            # Find the corresponsing grid position of p.
            base = (x[p] * inv_dx - 0.5).cast(int)
            fx = x[p] * inv_dx - base.cast(float)
            # Quadratic kernels, borrowed from mpm99
            w = [0.5 * (1.5 - fx)**2, 0.75 - (fx - 1)**2, 0.5 * (fx - 0.5)**2]
            # F[p]: deformation gradient update
            F[p] = (ti.Matrix.identity(float, 3) + dt * C[p]) @ F[p]
            # h: Hardening coefficient, borrowed from mpm99
            h = ti.exp(10 * (1.0 - Jp[p]))
            # Most parts of the following code are borrowed from mpm99
    ...
    for I in ti.grouped(grid_m):
        ...
        if grid_m[I] > 0:
            ...
            # Boundary
            i, j, k = I[0], I[1], I[2]
            if i < 3 and grid_v[I][0] < 0:
                grid_v[I][0] = 0  # Boundary conditions
            if i > n_grid - 3 and grid_v[I][0] > 0:
                grid_v[I][0] = 0
            if j < 3 and grid_v[I][1] < 0:
                grid_v[I][1] = 0
            if j > n_grid - 3 and grid_v[I][1] > 0:
                grid_v[I][1] = 0
            if k < 3 and grid_v[I][2] < 0:
                grid_v[I][2] = 0
            if k > n_grid - 3 and grid_v[I][2] > 0:
                grid_v[I][2] = 0
    for p in x:
        if is_used[p]:
            ...  # do something here
